...
namespace DuckNukem.src {
    ...
    class DNRPG : SMG {
        public DNRPG(float xval, float yval) : base(xval, yval) {
            this._type = "gun";
            this._editorName = "RPG";
            this._bio = "The rocket launched weapon is great for dealing with enemies from a distance.";
            this._fireSound = GetPath("sounds/DNRPG.wav");
            this.graphic = new Sprite(GetPath("sprites/DNRPG.png"));
            this._ammoType = new ATMissile();
            this.ammo = 5;
            this._fullAuto = false;
            this._fireWait = 5f;
            this._kickForce = 5f;
            this.loseAccuracy = 0.75f;
